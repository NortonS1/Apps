...
library(dendextend)
library(circlize)
setwd("~/Desktop/R Work2/PUT FILES IN HERE")
...
My_Palette <- colorRamp2(c("navy","aliceblue","bisque","chocolate1","firebrick"))
...
as.matrix(ht) -> htm
log10(htm) -> htl
htl[!rowSums(!is.finite(htl)),] -> htl
apply(htl,2,scale) -> hts
...
ui <- shinyUI(navbarPage(title = "Analyses:",
...
tabPanel( title = "Sample Gating",
          sidebarLayout(
            sidebarPanel(
              textInput("scattertitle", "Insert title here",
                        value = "Delete this text and write your title"),
              strong("Plot 1 markers"),
              selectInput("markersx1", "x-axis", c(colnames(gt)), multiple = FALSE ),
              selectInput("markersy1", "y-axis", c(colnames(gt)), multiple = FALSE ),
              actionButton(inputId = "plotscatter", label = "Plot Scatter Graph"),
              p(),
              p(strong("Plot 2 markers")),
              selectInput("markersx2", "x-axis", c(colnames(gt)), multiple = FALSE ),
              selectInput("markersy2", "y-axis", c(colnames(gt)), multiple = FALSE ),
              actionButton(inputId = "plotscatter2", label = "Plot 2"),
              p(),
              p(strong("Plot 3 markers")),
              selectInput("markersx3", "x-axis", c(colnames(gt)), multiple = FALSE ),
              selectInput("markersy3", "y-axis", c(colnames(gt)), multiple = FALSE ),
              actionButton(inputId = "plotscatter3", label = "Plot 3")
            ),
            mainPanel(
              plotOutput("scatter1", brush = "gate1"),
              plotOutput("scatter2", brush = "gate2"),
              plotOutput("scatter3", brush = "gate3"),
              textOutput("test")
            )
          )
),
  ...
  output$contour <- renderPlot({
    ...
    observeEvent(input$plotscatter, {
      ...
      output$scatter1 <- renderPlot({
        plot(log10(gt[,c(input$markersx1)]),
             log10(gt[,c(input$markersy1)]),
             xlab = input$markersx1,
             ylab = input$markersy1,
             pch = 20,
    ...
    observeEvent(input$plotscatter2, {
      
      gt1 <- reactive({gt[(gt[,c(input$markersx1)]>(10^input$gate1$xmin) & gt[,c(input$markersx1)]<10^(input$gate1$xmax) &
                             gt[,c(input$markersy1)]>(10^input$gate1$ymin) & gt[,c(input$markersy1)]<10^(input$gate1$ymax)),]})
      
      output$scatter2 <- renderPlot({
        plot(log10(gt1()[,c(input$markersx2)]),log10(gt1()[,c(input$markersy2)]),
             xlab = input$markersx2,
             ylab = input$markersy2,
             pch = 20,
             main = input$scattertitle,
             xlim = c(1,5),
             ylim = c(1,5))
    ...
    observeEvent(input$plotscatter3, {
      
      gt2 <- reactive({gt[(gt[,c(input$markersx2)]>(10^input$gate2$xmin) & gt[,c(input$markersx2)]<10^(input$gate2$xmax) &
                             gt[,c(input$markersy2)]>(10^input$gate2$ymin) & gt[,c(input$markersy2)]<10^(input$gate2$ymax)),]})
      
      output$scatter3 <- renderPlot({
        plot(log10(gt2()[,c(input$markersx3)]),log10(gt2()[,c(input$markersy3)]),
             xlab = input$markersx3,
             ylab = input$markersy3,
             pch = 20,
             main = input$scattertitle,
             xlim = c(1,5),
             ylim = c(1,5))
